...
define('CRLF', "\r\n");
$param=($argc > 1) ? $argv[1] : "";
include_once '/usr/share/moonfile/system/autoload.php';
...
if ($param=="install") {
    ...
    if (count($argv) >= 5) {
        ...
        if ($oSettings) {
            ...
            $poweredby = 'Powered by <a href="https://github.com/igor-moonlite/moonfile/" target="_blank">MoonFile</a> &copy; 2023';
    ...
    echo
    'Log into MoonFile admin interface at:'.CRLF.
    'http://localhost/moonfile/adminpanel/'.CRLF.
    'using login superadmin with empty password to configure the package.'.CRLF.
    'Users will be able to log into MoonFile at:'.CRLF.
    'http://localhost/moonfile/'.CRLF
    ;
